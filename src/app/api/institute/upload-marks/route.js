// src/app/api/institute/upload-marks/route.js
...
export async function POST(req) {
    ...
    try {
        ...
        // entries: [{ usn, subjects: [{ name, marksObtained, maxMarks }] }]
        ...
        for (const entry of entries) {
            try {
                ...
                if (existing) {
                    // Merge marks into existing subjects
                    for (const sub of subjects) {
                        const idx = existing.subjects.findIndex(s => s.name.toLowerCase() === sub.name.toLowerCase());
                        if (idx > -1) {
                            existing.subjects[idx].marksObtained = sub.marksObtained;
                            existing.subjects[idx].maxMarks = sub.maxMarks ?? existing.subjects[idx].maxMarks;
                        } else {
        ...
        return NextResponse.json({
            message: `Marks updated for ${updatedCount} student(s).`,
            count: updatedCount,
            errors,
        }, { status: 200 });
